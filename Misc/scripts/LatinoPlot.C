...
class LatinoPlot {
    ...
    public: 
        LatinoPlot() { _hist.resize(nSamples,0); _data = 0; _breakdown = false; _mass = 0; _nbins=_low=_high=-1;}
        ...
        void setMCHist   (const samp &s,        TH1F * h)  { 
            ...
            if (h->GetNbinsX() != _nbins || h->GetXaxis()->GetBinLowEdge(1) != _low || h->GetBinLowEdge(h->GetNbinsX()+1) != _high) {
        ...
        void Draw(TCanvas *c1, const int &rebin=1, const bool &div=false) {
            ...
            if(div) {
                ...
                AxisFonts(rref->GetYaxis(), "y", "ratio",1.56521739);
                AxisFonts(rref->GetXaxis(), "x", hstack->GetXaxis()->GetTitle(),1.56521739);
                rref->SetLabelSize(0.06*1.5621739,"XY");
                rref->SetTitleSize(0.06*1.5621739,"XY");
                rref->Draw("E2");
                line->Draw();
        ...
        THStack* GetStack(bool isLog) {
            ...
            if(_breakdown) {
                ...
                if(_units.Sizeof() == 1) {
                    ...
                    THStackAxisFonts(hstack, "x", TString::Format("%s [%s]",_xLabel.Data(),_units.Data()));
                    THStackAxisFonts(hstack, "y", TString::Format("entries / %.0f %s", _data->GetBinWidth(0),_units.Data()));
                }
            }
            return hstack;
        }
    ...
    private: 
        ...
        void DrawLabels() {
            ...
            if(_data        ) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _data,                  " data",                "lp"); j++; }
            if(_hist[iHWW  ]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iHWW  ]         , higgsLabel,             "l" ); j++; }
            if(_hist[iWW   ]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iWW   ]         , " WW",                  "f" ); j++; }
            if(_hist[iZJets]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iZJets]         , " Z+jets",              "f" ); j++; }
            if(_hist[iTop  ]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iTop  ]         , " top",                 "f" ); j++; }
            if(_hist[iVV   ]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iVV   ]         , " WZ/ZZ",               "f" ); j++; }
            if(_hist[iWJets]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iWJets]         , " W+jets",              "f" ); j++; }
            if(_hist[iWZ   ]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iWZ   ]         , " WZ",                  "f" ); j++; }
            if(_hist[iZZ   ]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iZZ   ]         , " ZZ",                  "f" ); j++; }
            if(_hist[iFakes]) { DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _hist[iFakes]         , " fakes",               "f" ); j++; }
            for(size_t i=0;i<_autreHists.size();++i) {
                                DrawLegend(pos[j], 0.84 - off[j]*_yoffset, _autreHists[i].second , _autreHists[i].first,   "f" ); j++; 
            }
