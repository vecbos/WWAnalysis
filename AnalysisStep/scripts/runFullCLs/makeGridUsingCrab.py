...
parser.add_option("-m", "--mass",  dest="mass",        default=0, type="int",    help="Higgs Mass")
...
for i,x in enumerate(points):
    ...
    if options.smart:
        if i < 0.25 * options.points:
            ...
            toys = "$(( 2 * $n ))";
    script.write("{cond} ./combine {wsp} -M HybridNew {opts} -m {mass} --fork {fork} -T {T} --clsAcc 0 -v {v} -n {out} --saveHybridResult --saveToys -s {seed} -i {toys} --singlePoint {x}\n".format(
