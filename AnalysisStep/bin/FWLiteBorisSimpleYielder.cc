...
typedef fwlite::AnalyzerWrapper<BorisSimpleYielder> WrappedFWLiteAnalyzer;
...
int main(int argc, char* argv[]) 
{
  ...
  WrappedFWLiteAnalyzer ana(edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process"), 
			    std::string("myAnalyzer"), std::string("tfileFolder"));
  ana.beginJob();
  ana.analyze();
  ana.endJob();
  return 0;
