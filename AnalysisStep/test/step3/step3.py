...
if len(args) > 0:
    if args[0] in [ 'SingleElectron', 'DoubleElectron', 'SingleMuon', 'DoubleMuon', 'MuEG']:
        ...
        dataset = ['MC','ggH', int(args[0].replace('ggH','')) ]
    else:
        dataset = ['MC', args[0]]
...
if dataset[0] == "MC":
    ...
    if dataset[1] == "ggH":
        process.higgsPt = cms.EDProducer("HWWKFactorProducer",
            genParticles = cms.untracked.InputTag("onlyHiggsGen"),
            inputFilename = cms.untracked.string("WWAnalysis/Misc/Scales/scalefactor.mh%d.dat" % dataset[2]),
            ProcessID = cms.untracked.int32(0), #10010), # 0: don't check
