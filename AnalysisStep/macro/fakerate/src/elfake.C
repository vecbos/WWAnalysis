...
void dofakes(const char* path, const char* outfilename) {
    ...
    for (int i = 0; i < tree->GetEntries(); i++) {
        ...
        if (mhits <= 1 && zmass>60 && zmass<120) {
            if (fabs(eta)<1.479) {
                ...
                hall3->Fill(pt);
                hall->Fill(pt,0.5);
                if (pt <= 10.0 && ((fabs(eta)< 0.8 && id > 0.47) || (fabs(eta)>=0.8 && id>0.04)) && iso<0.4) {hpass1->Fill(pt); hpass3->Fill(pt);hpass->Fill(pt, 0.5);} 
                if (pt >  10.0 && ((fabs(eta)< 0.8 && id > 0.50) || (fabs(eta)>=0.8 && id>0.12)) && iso<0.4) {hpass1->Fill(pt); hpass3->Fill(pt);hpass->Fill(pt, 0.5);} 
            }                  
            else {
                hall2->Fill(pt);
                hall3->Fill(pt);
                hall->Fill(pt,1.5);
                if (pt <= 10.0 && id>0.295 && iso<0.4) {hpass2->Fill(pt); hpass3->Fill(pt);hpass->Fill(pt, 1.5);}
                if (pt >  10.0 && id>0.600 && iso<0.4) {hpass2->Fill(pt); hpass3->Fill(pt);hpass->Fill(pt, 1.5);}
            }                  
        }
    ...
    prep(hfake1, text1,  canvas, 1, "Electrons In Barrel"); 
    prep(hfake2, text2,  canvas, 2, "Electrons In Endcap"); 
    prep(hfake3, text3,  canvas, 3, "Electrons In Barrel + Endcap"); 
...
void elfake() {
    ...
    dofakes("/home/avartak/CMS/Higgs/CMSSW_4_2_8_patch7/src/WWAnalysis/AnalysisStep/trees/conf2/hzzTree.root", "elfakes");
