...
##
    ...
    def applyMetXYshift(self):
        ...
        chmet     = numpy.ones(1, dtype=numpy.float32)
        chmetphi  = numpy.ones(1, dtype=numpy.float32)
        pchmet    = numpy.ones(1, dtype=numpy.float32)
...
##
            ...
            chmetx = self.oldttree.chmet * cos (self.oldttree.chmetphi)
            chmety = self.oldttree.chmet * sin (self.oldttree.chmetphi)
            ...
            ###
            ####
            #### minus! for consistency with previous procedure 
            ####
            ####
            newchmetx = chmetx - dchmetx
            newchmety = chmety - dchmety
            ####
            ####
            ###
            ##
            ...
            # correct chmet
            newchmet = sqrt(newchmetx*newchmetx + newchmety*newchmety)

            vchmet = ROOT.TLorentzVector()
            vchmet.SetPxPyPzE(newchmetx, newchmety, 0, newchmet)

            chmet   [0]  = vchmet.Pt()
            chmetphi[0]  = vchmet.Phi()
            ...
            dphilchmet1 = deltaPhi( l1  ,  vchmet)
            dphilchmet2 = deltaPhi( l2  ,  vchmet)
            dphilchmet  = min (dphilchmet1, dphilchmet2)


            if dphilchmet < math.pi/2.:
               pchmet   [0] = chmet[0] * sin(dphilchmet)
            else :
               pchmet   [0] = chmet[0]
